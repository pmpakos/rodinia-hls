...
void bpnn_adjust_weights_FPGA(float *hidden, float *input, float *weight, float *prev_weight, cl_context& context, cl_command_queue& commands, cl_program& program, cl_kernel& kernel) {
  printf("Backprop (back) INPUT_SIZE=%d\n", INPUT_SIZE);
  ...
  d_weight = clCreateBuffer(context, CL_MEM_READ_WRITE, (INPUT_SIZE+1) * 17 * sizeof(float), NULL, &err);
  if (err != CL_SUCCESS) { printf("ERROR: clCreateBuffer d_weight (size:(INPUT_SIZE+1)*17) => %d\n", err);}
  d_prev_weight = clCreateBuffer(context, CL_MEM_READ_WRITE, (INPUT_SIZE+1) * 17 * sizeof(float), NULL, &err);
  if (err != CL_SUCCESS) { printf("ERROR: clCreateBuffer d_prev_weight (size:(INPUT_SIZE+1)*17) => %d\n", err); }
  ...
  err = clEnqueueWriteBuffer(commands, d_weight, 1, 0, (INPUT_SIZE+1) * 17 * sizeof(float), weight, 0, 0, 0);
  if (err != CL_SUCCESS) { printf("ERROR: clEnqueueWriteBuffer d_weight (size:(INPUT_SIZE+1)*17) => %d\n", err); }
  err = clEnqueueWriteBuffer(commands, d_prev_weight, 1, 0, (INPUT_SIZE+1) * 17 * sizeof(float), prev_weight, 0, 0, 0);
  if (err != CL_SUCCESS) { printf("ERROR: clEnqueueWriteBuffer d_prev_weight (size:(INPUT_SIZE+1)*17) => %d\n", err);}
  ...
  err = clEnqueueReadBuffer(commands, d_weight, 1, 0, (INPUT_SIZE+1) * 17 * sizeof(float), weight, 0, 0, 0);
  if (err != CL_SUCCESS) { printf("ERROR: Memcopy Out\n"); }

  err = clEnqueueReadBuffer(commands, d_prev_weight, 1, 0, (INPUT_SIZE+1) * 17 * sizeof(float), prev_weight, 0, 0, 0);
  if (err != CL_SUCCESS) { printf("ERROR: Memcopy Out\n"); }
