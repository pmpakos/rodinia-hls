...
#define INPUT_SIZE 327680
#define INPUT_SIZE_P1 327681
...
void bpnn_train_kernel(BPNN *net, float *eo, float *eh, cl_context& context, cl_command_queue& commands, cl_program& program, cl_kernel& kernel) {
    ...
    float *input_weights_FPGA = (float *)malloc(INPUT_SIZE_P1*17*sizeof(float));
    float *input_prev_weights_FPGA = (float *)malloc(INPUT_SIZE_P1*17*sizeof(float));
    for (int j = 0; j <= 16; j++) {
...
void setup(cl_context& context, cl_command_queue& commands, cl_program& program, cl_kernel& kernel) {
    ...
    float out_err, hid_err;
    net = bpnn_create(INPUT_SIZE, 16, 1); // (16, 1 can not be changed)
    load(net);
