...
void buffer_compute(int flag, int kk, float l1_buf[INPUT_SIZE_1], float conn_buf[TILE_SIZE][16], float sum[UNROLL_FACTOR][16]) {
#pragma HLS INLINE off
    int i, j, k;
    if (flag) {
...
#pragma HLS ARRAY_PARTITION variable=l2_buf complete dim=1
            ...
            memcpy_wide_bus_read_float(l1_buf, l1, sizeof(float), sizeof(float) * INPUT_SIZE);
