...
void copy_in(uint64_t* large_A, char* part_buf_A) {
  ...
  for (i=0; i<JOBS_PER_PE; i++) {
    ...
    #pragma HLS PIPELINE
      ...
      part_buf_A[i*BLEN+j*8+0] = (uint8_t)(curr >> 0);
      part_buf_A[i*BLEN+j*8+1] = (uint8_t)(curr >> 8);
      part_buf_A[i*BLEN+j*8+2] = (uint8_t)(curr >> 16);
      part_buf_A[i*BLEN+j*8+3] = (uint8_t)(curr >> 24);
      part_buf_A[i*BLEN+j*8+4] = (uint8_t)(curr >> 32);
      part_buf_A[i*BLEN+j*8+5] = (uint8_t)(curr >> 40);
      part_buf_A[i*BLEN+j*8+6] = (uint8_t)(curr >> 48);
      part_buf_A[i*BLEN+j*8+7] = (uint8_t)(curr >> 56);
    }
  }
}
...
void copy_out(uint64_t* large_E, char* part_buf_E) {
  ...
  for (i=0; i<JOBS_PER_PE; i++) {
    for (j=0; j<16; j++) {
    #pragma HLS PIPELINE
      uint64_t curr = part_buf_E[i*BLEN+j*8+7];
      curr = (curr << 8) | part_buf_E[i*BLEN+j*8+6];
      curr = (curr << 8) | part_buf_E[i*BLEN+j*8+5];
      curr = (curr << 8) | part_buf_E[i*BLEN+j*8+4];
      curr = (curr << 8) | part_buf_E[i*BLEN+j*8+3];
      curr = (curr << 8) | part_buf_E[i*BLEN+j*8+2];
      curr = (curr << 8) | part_buf_E[i*BLEN+j*8+1];
      curr = (curr << 8) | part_buf_E[i*BLEN+j*8+0];
      large_E[i*16+j] = curr;
    }
  }
}
